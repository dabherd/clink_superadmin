...
abstract class PayPalDigitalGoods {
	/**
	 * Ab array of name value pairs representing the seller's API Username, Password & Signature.
	 *
	...
	/**
	 * The PayPal URL for initiating the popup payment process.
	...
	/**
	 * Stores the token oncei it has been acquired from PayPal 
	 */
	protected $token;
	...
	public function __construct($args = array()) {
		if( '' == PayPalDigitalGoodsConfiguration::username() || '' == PayPalDigitalGoodsConfiguration::password() || '' == PayPalDigitalGoodsConfiguration::signature() )
			exit( 'You must specify your PayPal API username, password & signature in the $api_credentials array. For details of how to ' );
		elseif( ( empty( $args['return_url'] ) && '' == PayPalDigitalGoodsConfiguration::username() ) || ( empty( $args['cancel_url'] ) && '' == PayPalDigitalGoodsConfiguration::cancel_url() ) )
		...
		$defaults = array(
			...
			'business_name' => '',
			'solution_type' => 'Sole',
			'return_url' => PayPalDigitalGoodsConfiguration::return_url(),
			'cancel_url' =>
			PayPalDigitalGoodsConfiguration::cancel_url(),
			'notify_url' => PayPalDigitalGoodsConfiguration::notify_url(),
			'local_code' => PayPalDigitalGoodsConfiguration::local_code(),

			);

		$args = array_merge($defaults, $args);

		$this->currency = PayPalDigitalGoodsConfiguration::currency();
		...
		$this->local_code = $args['local_code'];
	}
	...
	protected function get_api_credentials_url() {
		return 'USER=' . urlencode( PayPalDigitalGoodsConfiguration::username()).
		'&PWD=' . urlencode(
			PayPalDigitalGoodsConfiguration::password()).
		'&SIGNATURE=' . urlencode(
			PayPalDigitalGoodsConfiguration::signature()). 
		'&VERSION=' . urlencode(
			PayPalDigitalGoodsConfiguration::version());
	}

	/**
	 * @param $action, stirng. The Paypal NVP API action to create the URL for. One of SetExpressCheckout,
	 CreateRecurringPaymentsProfile or GetRecurringPaymentsProfile or GetReccuringPaymentsProfileDetails.
	 * @param $profile_id (optional) string. A PayPal GetReccuring Payment Profile ID, required for GetRecurringPaymentsProfileDetails operation.
	 * @return string A URL which cna be called with the @seecall_paypal() method to perform the appropriate API operation.
	 */
	 protected function get_payment_details_url($action, $profile_or_transaction_id = '') {
	 	...
	 	// Parameters to Request Reccuring Payment Token
	 	if('SetExpressCheckout' == $action) {
	 		...
	 		. '&LOCALECODE=' .urlencode($this->local_code)
	  ...
	  * 
	  * It passes payment details of the items pruchased and therefore, set_payment_details() must have been called before this function.
	  *
	  * Return:
	  */
	  ...
	  *
	  * Called internally on return from set_express_checkout(). Can be called anytime to get details of a transaction for which you have the Token.
	  */
	  ...
	  *
	  * @param action, string, required. The API operation to be performed, eg. GetExpressCheckoutDetails. The action is abstracted from the developer by the appropriate helper function eg. GetExpressCheckoutDetails via get_checkout_details()
	  */
	 protected function call_paypal($action, $profile_id = '', $status = '', $args = array()) {
	 	...
	 	curl_setopt($ch, CURLOPT_URL,
	 		PayPalDigitalGoodsConfiguration::endpoint());
	 	...
	 	if (! $response)
	 		exit($action . 'failed:' . curl_error($ch) . '
	 			('.curl_errno($ch).')');
	 	...
	 	if ((0 == sizeof($parsed_response)) || ! array_key_exists('ACK', $parsed_response)) {
	 		exit("Invalid HTTP Response for POST request($api_parameters) to".
	 			PayPalDigitalGoodsConfiguration::endpoint());
	 	}
	 	 ...
	 	 *
	 	 * No need to call this function manually, required scripts are automatically printed with @see print_buy_button().
...
return $dg_script;
		...
		/**
		 * Create and return the Buy (or Subscribe) button for the page.
		...
		public function get_buy_button($args = array()) {
			...
			$defaults = array(
				'id'        => 'paypal-submit',
				'type'      => 'anchor',
				'href'      => PayPalDigitalGoodsConfiguration::checkout_url(),
				'alt'       => 'Submit',
				'get_token' => true,
				'locale'    => 'en_US'
				);
			...
			if ($args['type'] == 'anchor') {
				...
				// Include the token in the href if the default href is not overriden
				if ($args['href'] ==
					PayPalDigitalGoodsConfiguration::checkout_url())
	 ...
	 */
	 ...
	 /**
	  * Returns the Checkout URL includeing a token for this transaction.
	  */
	 public function get_checkout_url() {
	 	...
	 	return PayPalDigitalGoodsConfiguration::checkout_url().$this->token;
	 }
	 /**
